...
func svcDiskCreate(spdkClient *spdkclient.Client, diskName, diskUUID, diskPath, diskDriver string, blockSize int64) (ret *spdkrpc.Disk, err error) {
	...
	exactDiskDriver, err := disk.GetDiskDriver(diskDriver, diskPath)
	if err != nil {
...
func svcDiskDelete(spdkClient *spdkclient.Client, diskName, diskUUID, diskDriver string) (ret *emptypb.Empty, err error) {
	log := logrus.WithFields(logrus.Fields{
		"diskName":   diskName,
		"diskUUID":   diskUUID,
		"diskDriver": diskDriver,
	...
	if _, err := disk.DiskDelete(spdkClient, aioBdevName, diskDriver); err != nil {
...
func svcDiskGet(spdkClient *spdkclient.Client, diskName, diskPath, diskDriver string) (ret *spdkrpc.Disk, err error) {
	...
	var exactDiskDriver disk.BlockDiskDriver
	...
	for i, bdev := range bdevs {
		...
		case spdktypes.BdevProductNameAio:
			...
			exactDiskDriver = disk.BlockDiskDriverAio
		case spdktypes.BdevProductNameVirtioBlk:
			exactDiskDriver = disk.BlockDiskDriverVirtioBlk
			targetBdev = &bdevs[i]
		case spdktypes.BdevProductNameVirtioScsi:
			exactDiskDriver = disk.BlockDiskDriverVirtioScsi
			targetBdev = &bdevs[i]
		case spdktypes.BdevProductNameNvme:
			exactDiskDriver = disk.BlockDiskDriverNvme
			targetBdev = &bdevs[i]
		}
		if targetBdev != nil {
			break
...
func validateAioDiskCreation(spdkClient *spdkclient.Client, diskPath string, diskDriver disk.BlockDiskDriver) error {
...
func addBlockDevice(spdkClient *spdkclient.Client, diskName, diskUUID, originalDiskPath string, diskDriver disk.BlockDiskDriver, blockSize int64) (string, error) {
	log := logrus.WithFields(logrus.Fields{
		"diskName":   diskName,
		"diskUUID":   diskUUID,
...
func lvstoreToDisk(spdkClient *spdkclient.Client, diskPath, lvstoreName, lvstoreUUID string, diskDriver disk.BlockDiskDriver) (*spdkrpc.Disk, error) {
	...
	diskID := diskPath
	if diskDriver == disk.BlockDiskDriverAio {
		diskID, err = getDiskID(getDiskPath(diskPath))
		if err != nil {
