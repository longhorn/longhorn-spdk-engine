...
func (d *Disk) DiskDelete(spdkClient *spdkclient.Client, diskName, diskUUID, diskPath, diskDriver string) (ret *emptypb.Empty, err error) {
	...
	if diskUUID != "" {
		...
		if len(lvstores) == 0 {
			return nil, errors.Wrapf(err, "lvstores get nothing with UUID %v", diskUUID)
		}

		lvstore := &lvstores[0]
		if lvstore.UUID != diskUUID {
			log.Warnf("Disk uuid %v does not match lvstore uuid %v", diskUUID, lvstore.UUID)
			return nil, grpcstatus.Errorf(grpccodes.NotFound, "disk uuid %v does not match lvstore uuid %v", diskUUID, lvstore.UUID)
		}
	} else {
