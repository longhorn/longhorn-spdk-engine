...
func (s *TestSuite) TestEngineRestoreCloseVolumeDevSyncFailureFailsRestore(c *C) {
	...
	closeCalled := false
	err = r.closeVolumeDev(volDev, func() error {
		closeCalled = true
		c.Assert(r.State, Not(Equals), btypes.ProgressStateError)
		c.Assert(r.Error, Equals, "")
		return nil
	})
	c.Assert(err, NotNil)
...
func (s *TestSuite) TestEngineRestoreCloseVolumeDevCloseFailureFailsRestore(c *C) {
	fmt.Println("Testing EngineRestore reports a failed device close as a failed restore")
	...
	c.Assert(r.State, Equals, btypes.ProgressStateError)
	c.Assert(strings.Contains(r.Error, "failed to close NVMe device"), Equals, true)
	c.Assert(r.volumeDevCloseSucceeded, Equals, false)
	c.Assert(r.VolumeDevClosed, Equals, false)
...
func (s *TestSuite) TestEngineRestoreCloseVolumeDevSyncAndCloseFailureFailsRestore(c *C) {
	fmt.Println("Testing EngineRestore reports both failed device sync and close")
	...
	err = r.closeVolumeDev(volDev, func() error {
		...
		c.Assert(r.State, Not(Equals), btypes.ProgressStateError)
		c.Assert(r.Error, Equals, "")
		return errors.New("close input/output error")
	})
	c.Assert(err, NotNil)
	c.Assert(closeCalled, Equals, true)

	c.Assert(r.State, Equals, btypes.ProgressStateError)
	c.Assert(strings.Contains(r.Error, "failed to sync NVMe device"), Equals, true)
	c.Assert(strings.Contains(r.Error, "failed to close NVMe device"), Equals, true)
	c.Assert(r.volumeDevCloseSucceeded, Equals, false)
	c.Assert(r.VolumeDevClosed, Equals, false)
}

func (s *TestSuite) TestWaitForRestoreCompleteWaitsForVolumeDevClose(c *C) {
	fmt.Println("Testing restore completion waits for the volume device to close")
	...
	c.Assert(e.waitForRestoreCompleteOnce(), NotNil)
}

func (s *TestSuite) TestWaitForRestoreCompleteWaitsForFinalStatusAfterClose(c *C) {
	fmt.Println("Testing restore completion waits for final status after closing the volume device")
	...
	c.Assert(e.restore.closeVolumeDev(volDev, volDev.Close), IsNil)
	c.Assert(e.waitForRestoreCompleteOnce(), NotNil)

	e.restore.UpdateRestoreStatus(volDev.Name(), 100, nil)
	c.Assert(e.waitForRestoreCompleteOnce(), IsNil)
}
...
func (s *TestSuite) TestWaitForRestoreCompleteSucceedsAtFullProgress(c *C) {
	...
	e.restore = NewEngineRestore(nil, "s3://backupbucket@us-east-1/backupstore?backup=backup-2", "backup-2", e, nil)
	e.restore.Progress = 100
	e.restore.volumeDevCloseSucceeded = true
	e.restore.UpdateRestoreStatus("vol-a", 100, nil)
