...
func (r *Replica) SnapshotHash(spdkClient *spdkclient.Client, snapshotName string, rehash bool) (err error) {
	...
	// Snapshots whose parent is a backing image are esnap clones backed by an external bdev.
	// Computing a checksum for them is not meaningful, so skip it.
	if types.IsBackingImageSnapLvolName(snapSvcLvol.Parent) {
		r.log.Debugf("Skipping snapshot hash for %s(%s) since it is an esnap clone backed by backing image %s", snapshotName, snapLvolName, snapSvcLvol.Parent)
		return nil
	}

	// Snapshots that rely on an external snapshot bdev during rebuilding are also esnap clones backed by an external bdev.
	// Computing a checksum for them triggers reads through the external snapshot bdev's I/O channel.
	// If the checksum goroutine is still running when doCleanupForRebuildingDst disconnects that bdev,
	// the pending I/O causes SPDK assertion: TAILQ_EMPTY(&ch->io_submitted) failed.
	// Guard on the bdev name (not isRebuilding) because the hash goroutine runs without the replica lock.
	if r.rebuildingDstCache.externalSnapshotBdevName != "" {
		r.log.Debugf("Skipping snapshot hash for %s(%s) since the replica chain has an external snapshot bdev %s", snapshotName, snapLvolName, r.rebuildingDstCache.externalSnapshotBdevName)
		return nil
	}
