...
func (r *Replica) SnapshotHash(spdkClient *spdkclient.Client, snapshotName string, rehash bool) (err error) {
	...
	// Computing a checksum for them triggers full network reads from the remote source replica, which is wasteful and triggers assertions.
	if r.isRebuilding && r.rebuildingDstCache.externalSnapshotName != "" {
		r.log.Debugf("Skipping snapshot hash for %s(%s) since the replica is rebuilding with an external snapshot", snapshotName, snapLvolName)
		return nil
	}
