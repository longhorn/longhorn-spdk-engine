			...
			if candidateReplicaName != replicaName {
				if candidateReplicaName != "" {
					...
					if IsBackingImageSnapLvolName(candidateReplicaAncestorName) {
						...
						if err != nil {
							logrus.Warnf("BUG: ancestor name %v is from backingImage.Snapshot lvol name, it should be a valid backing image lvol name", candidateReplicaAncestorName)
						}
					}
					if IsBackingImageSnapLvolName(currentReplicaAncestorName) {
						...
						if err != nil {
							logrus.Warnf("BUG: ancestor name %v is from backingImage.Snapshot lvol name, it should be a valid backing image lvol name", currentReplicaAncestorName)
						}
					}
