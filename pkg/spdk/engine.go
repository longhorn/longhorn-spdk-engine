...
// Create builds the engine's bdev_raid1 over the backends in
// replicaAddressMap. backendFactory selects the backend implementation
// (replicaBackend for RAID1, shardGroupBackend for EC); it is the only
...
func (e *Engine) Create(spdkClient *spdkclient.Client, replicaAddressMap map[string]string, portCount int32, superiorPortAllocator *commonbitmap.Bitmap,
	salvageRequested bool, backendFactory BackendFactory) (ret *spdkrpc.Engine, err error) {
	...
	replicaBdevList := e.connectReplicas(spdkClient, replicaAddressMap, backendFactory)
...
func (e *Engine) createNVMeTCPTarget(spdkClient *spdkclient.Client, superiorPortAllocator *commonbitmap.Bitmap, portCount int32, initialANAState NvmeTCPANAState) error {
	...
	if err := e.startExposeNVMeTCPTargetLocked(spdkClient, initialANAState, spdkANAState, cntlid); err != nil {
...
func (e *Engine) startExposeNVMeTCPTargetLocked(spdkClient *spdkclient.Client, anaState NvmeTCPANAState, spdkANAState spdktypes.NvmfSubsystemListenerAnaState, cntlid uint16) error {
...
// backends. The backendFactory builds the layout-specific Backend
// implementation (replicaBackend or shardGroupBackend) - connectReplicas
// itself stays layout-blind. It returns the list of successfully connected
// bdev names.
func (e *Engine) connectReplicas(spdkClient *spdkclient.Client, replicaAddressMap map[string]string, backendFactory BackendFactory) []string {
	...
	for replicaName, replicaAddr := range replicaAddressMap {
		...
		if err != nil {
			...
			e.backends[replicaName].SetMode(types.ModeRW)
			e.backends[replicaName].SetBdevName(bdevName)
...
func (e *Engine) SnapshotClone(snapshotName, srcEngineName, srcEngineAddress string, cloneMode spdkrpc.CloneMode, dstReplicaSrcReplicaPairMap map[string]string) (err error) {
	...
	if cloneMode != spdkrpc.CloneMode_CLONE_MODE_LINKED_CLONE {
		...
		if len(e.backends) != 1 {
			return fmt.Errorf("destination engine must only have 1 replica when doing deep-copy snapshot clone. Current replica count is %v", len(e.backends))
		}
	}
...
func (e *Engine) snapshotCloneLinkedN(snapshotName string, dstReplicaSrcReplicaPairMap map[string]string, srcReplicaCandidates map[string]replicaCandidate) error {
	...
	for rName, rStatus := range e.backends {
		if dstReplicaSrcReplicaPairMap[rName] == "" {
			rStatus.SetMode(types.ModeERR)
			e.log.Warnf("Dst replica %s is not included in the dst-src replica pair map for linked-clone, marking it ERR", rName)
		}
	}
	for dstName, srcName := range dstReplicaSrcReplicaPairMap {
		dstStatus, ok := e.backends[dstName]
		if !ok || dstStatus == nil {
			e.log.Warnf("Dst replica %s not found in engine backends map, skipping", dstName)
			continue
		}
		if dstStatus.Mode() != types.ModeRW {
			dstStatus.SetMode(types.ModeERR)
			e.log.Warnf("Dst replica %s is not RW (mode=%v), marking it ERR for linked-clone", dstName, dstStatus.Mode())
			continue
		}
	...
	var mu sync.Mutex
	cloneErrs := make(map[string]error, len(dstEntries))
	...
	for _, entry := range dstEntries {
		...
		go func(dst dstEntry) {
			...
			if err != nil {
				mu.Lock()
				cloneErrs[dst.name] = errors.Wrapf(err, "failed to get client for dst replica %s", dst.name)
				mu.Unlock()
				return
			}
			defer func() {
			...
			if err := cli.ReplicaSnapshotCloneDstStart(dst.name, snapshotName, dst.srcReplicaName, dst.srcReplicaAddr, spdkrpc.CloneMode_CLONE_MODE_LINKED_CLONE); err != nil {
				mu.Lock()
				cloneErrs[dst.name] = errors.Wrapf(err, "dst replica %s: SnapshotCloneDstStart failed", dst.name)
				mu.Unlock()
			}
		}(entry)
	}
	...
	// Mark failed replicas as ERR so the manager can schedule rebuilds for them.
	var errList []error
	for replicaName, cloneErr := range cloneErrs {
		if rStatus, ok := e.backends[replicaName]; ok && rStatus != nil {
			rStatus.SetMode(types.ModeERR)
		}
		e.log.WithError(cloneErr).Errorf("Dst replica %s failed during N-replica linked-clone", replicaName)
		errList = append(errList, cloneErr)
	}
	...
	if len(cloneErrs) > 0 {
		e.log.Warnf("%d/%d dst replicas failed during linked-clone and have been marked ERR for later rebuild",
			len(cloneErrs), len(dstEntries))
	}
	return nil
}
...
func (e *Engine) Expand(spdkClient *spdkclient.Client, size uint64) (err error) {
	...
	case types.FrontendSPDKTCPBlockdev, types.FrontendSPDKTCPNvmf:
		...
		if err := e.startExposeNVMeTCPTargetLocked(spdkClient, currentANAState, spdkANAState, cntlid); err != nil {
			return errors.Wrapf(err, "failed to start exposing RAID bdev for engine target %v", e.Name)
		}
...
func (e *Engine) resolveReplicaAncestor(replicaName string, view *BackendView,
	...
	if len(view.Snapshots) > 0 {
		...
		for _, snapApiLvol := range view.Snapshots {
			if snapApiLvol.Parent == "" || view.Snapshots[snapApiLvol.Parent] == nil {
