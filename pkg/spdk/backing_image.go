...
func (bi *BackingImage) ValidateAndUpdate(spdkClient *spdkclient.Client) (err error) {
	...
	lvsName := GetLvsNameByUUID(spdkClient, bi.LvsUUID)
	if lvsName == "" {
		return fmt.Errorf("failed to get the lvs name for backing image %v with lvs uuid %v", bi.Name, bi.LvsUUID)
	}
	...
	state := GetSnapXattr(spdkClient, bi.Alias, types.BackingImageSnapshotAttrPrepareState)
	if state == "" {
		return fmt.Errorf("failed to get the prepare state for backing image snapshot %v", bi.Name)
	}
...
func (bi *BackingImage) prepareFromSync(targetFh *os.File, fromAddress, srcLvsUUID string) (err error) {
	...
	exposedSnapshotLvolAddress, err := srcBackingImageServiceCli.BackingImageExpose(bi.ctx, bi.Name, srcLvsUUID)
	if err != nil {
	...
	_, _, err = connectNVMeTarget(srcIP, srcPort)
	if err != nil {
	...
	if err := srcBackingImageServiceCli.BackingImageUnexpose(bi.ctx, bi.Name, srcLvsUUID); err != nil {
