...
func (s *TestSuite) TestSPDKMultipleThread(c *C) {
	...
	for i := range concurrentCount {
		...
		go func() {
			...
			err = spdkCli.EngineFrontendReplicaAdd(engineFrontendName, replicaName3, net.JoinHostPort(ip, strconv.Itoa(int(replica3.PortStart))), defaultTestFastSync, nil)
			c.Assert(err, IsNil)
...
func (s *TestSuite) spdkMultipleThreadSnapshotOpsAndRebuilding(c *C, withBackingImage bool) {
	...
	for i := range concurrentCount {
		...
		go func() {
			...
			err = spdkCli.EngineFrontendReplicaAdd(engineFrontendName, replicaName4, net.JoinHostPort(ip, strconv.Itoa(int(replica4.PortStart))), defaultTestFastSync, nil)
			c.Assert(err, IsNil)
...
func (s *TestSuite) spdkMultipleThreadFastRebuilding(c *C, withBackingImage bool) {
	...
	for i := range concurrentCount {
		go func() {
			...
			err = spdkCli.EngineFrontendReplicaAdd(engineFrontendName, replicaName1, net.JoinHostPort(ip, strconv.Itoa(int(replica1.PortStart))), defaultTestFastSync, nil)
			c.Assert(err, IsNil)
...
func (s *TestSuite) TestSPDKEngineFrontendReplicaAdd(c *C) {
	...
	err = spdkCli.EngineFrontendReplicaAdd(engineFrontendName, replicaNames[1], replica2Address, defaultTestFastSync, nil)
	c.Assert(err, IsNil)
...
func (s *TestSuite) TestSPDKEngineReplicaAddWithoutEngineFrontendInfo(c *C) {
	...
	err = spdkCli.EngineReplicaAdd(engineName, replicaNames[1], replica2Address, defaultTestFastSync, "", "", nil)
	c.Assert(err, IsNil)
