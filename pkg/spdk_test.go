...
// connectToNVMfInitiator connects to the NVMe/TCP initiator.
func connectToNVMfInitiator(endpoint string, nqn string) error {
	...
	fmt.Println("Connecting to NVMe/TCP initiator: host", host, "port", port, "nqn", nqn)
	...
	if _, err := helperinitiator.ConnectTarget(host, port, nqn, ne); err != nil {
		return err
	}
	return nil
}

// disconnectFromNVMfInitiator disconnects from the NVMe/TCP initiator.
func disconnectFromNVMfInitiator(nqn string) error {
	fmt.Println("Disconnecting from NVMe/TCP initiator: nqn", nqn)
	...
	return helperinitiator.DisconnectTarget(nqn, ne)
}
...
func (s *TestSuite) TestSPDKEngineFrontendCreateWithoutEngine(c *C) {
	...
	volumeName := fmt.Sprintf("test-vol-%s", time.Now().Format("20060102150405"))
	...
	tests := []struct {
		...
		},
		{
			...
			verifyFrontend: func(c *C, _ string, engineName string, engine *api.Engine, engineFrontend *api.EngineFrontend) {
				...
				err := connectToNVMfInitiator(endpoint, nqn)
				c.Assert(err, IsNil)
				...
				err = disconnectFromNVMfInitiator(nqn)
				c.Assert(err, IsNil)
			},
		},
		{
