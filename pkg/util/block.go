...
func GetBlockDiskType(devName string) (string, error) {
	ne, err := helperutil.NewExecutor(commonTypes.ProcDirectory)
	if err != nil {
		return "", errors.Wrap(err, "failed to create executor")
	}

	cmdArgs := []string{"-n", "-d", "-o", "type", fmt.Sprintf("/dev/%s", devName)}
	output, err := ne.Execute(nil, "lsblk", cmdArgs, types.ExecuteTimeout)
	if err != nil {
		return "", errors.Wrap(err, "failed to get disk type")
	}

	return strings.TrimSpace(string(output)), nil
}

func GetBlockDiskSubsystems(devName string) ([]string, error) {
	ne, err := helperutil.NewExecutor(commonTypes.ProcDirectory)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create executor")
	}

	cmdArgs := []string{"-n", "-d", "-o", "subsystems", fmt.Sprintf("/dev/%s", devName)}
	output, err := ne.Execute(nil, "lsblk", cmdArgs, types.ExecuteTimeout)
	if err != nil {
